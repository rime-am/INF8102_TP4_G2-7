...
from troposphere.cloudwatch import Alarm, MetricDataQuery, MetricStat, MetricDimension, Metric


t = Template()
t.set_description("Question 3.2")
...
vpc_flow_log = t.add_resource(
    ec2.FlowLog(
        ...
        LogDestination="arn:aws:s3:::polystudents3-2346175"
    )
)

# Import du role LabRole
lab_role = t.add_resource(
    InstanceProfile(
        "LabInstanceProfile",
        Roles=["LabRole"]
    )
...
# On utilise la même configuration dans l'exemple json
# Création instance EC2 publique dans AZ1
public_instance_az1 = t.add_resource(
    ec2.Instance(
        ...
        IamInstanceProfile=Ref(lab_role),
        Tags=[{"Key": "Name", "Value": "PublicInstanceAZ1"}]
    )
)

# Création instance EC2 publique dans AZ2
public_instance_az2 = t.add_resource(
    ec2.Instance(
        ...
        SecurityGroupIds=[Ref(sg)],
        IamInstanceProfile=Ref(lab_role),
        Tags=[{"Key": "Name", "Value": "PublicInstanceAZ2"}]
    )
)

# Création instance EC2 privée dans AZ1
private_instance_az1 = t.add_resource(
    ec2.Instance(
        ...
        SecurityGroupIds=[Ref(sg)],
        IamInstanceProfile=Ref(lab_role),
        Tags=[{"Key": "Name", "Value": "PrivateInstanceAZ1"}]
    )
)

# Création instance EC2 privée dans AZ2
private_instance_az2 = t.add_resource(
    ec2.Instance(
        ...
        SecurityGroupIds=[Ref(sg)],
        IamInstanceProfile=Ref(lab_role),
        Tags=[{"Key": "Name", "Value": "PrivateInstanceAZ2"}]
    )
)

# Alarme Cloudwatch sur la MOYENNE des instances
instances = [public_instance_az1, public_instance_az2, private_instance_az1, private_instance_az2]

# Création de metriques pour chaque instance basé sur le nombre de paquets reçus
metric_queries = []

for i, instance_name in enumerate(instances, start=1):
        ...
        MetricDataQuery(
            ...
            MetricStat=MetricStat(
                Metric=Metric(
                    ...
                    Dimensions=[
                        MetricDimension(Name="InstanceId", Value=Ref(instance_name))
                    ]
                ),
                Period=60,
                Stat="Average",
...
# Calcul de la moyenne des quatres instances
metric_queries.append(
    MetricDataQuery(
        Id="avgPacketsIn",
        Expression="AVG([m1,m2,m3,m4])",
        Label="Average NetworkPacketsIn",
        ReturnData=True
    )
)
...
t.add_resource(
    Alarm(
        "TrafficAlarm",
        AlarmDescription="Alarme pour quand le trafic d'entrée moyen > 1000 pkts/sec",
        EvaluationPeriods=5,
        Threshold=1000,
...
with open("Q3.2/vpc_with_ec2.yaml", "w") as f:
    f.write(t.to_yaml())

print("Template générer avec succès pour la question 3.2!")
